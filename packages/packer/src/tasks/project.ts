...
import type { RoutesConfigGroup } from './config'
 ...
 * `routes module` is generated by your {@link UserConfig.routes} and the structure of your project.
 *
 * @public
 */
...
const getRoutes = (context: Context) => {
  const {
    config,
    CONSTANTS: { ROUTES },
  ...
  Object.keys(config.routes).forEach(
    (subpath) => {
      const p2rcgm: Record<string, RoutesConfigGroup> = {}
      config.routes[subpath].groups.forEach(
        (group) => {
          fg.sync(group.patterns).forEach((path) => (p2rcgm[path] = group))
        }
      )

      const paths = Object.keys(p2rcgm)
      ...
      Object.keys(pn2pm).forEach(
        (pn) => {
          const paths = pn2pm[pn]
          const length = paths.length
          if (!length) {
            return
          }
          let lca = paths[0].slice(0, paths[0].lastIndexOf('/'))
          ...
          paths.forEach(
            (path) => {
              const raw = p2rcgm[path].base + path.replace(lca, getPkgId(pn)).replace(/(\/index)?(\..+?)?$/, '')
              const re = config.routes[subpath].extends.find((re) => re.id === path)
      ...
      brs.forEach(
        (br) => {
          const rcg = p2rcgm[br.id]
          if (rcg.parent) {
            const parent = brs.find((br) => br.id === rcg.parent)
            if (!parent) {
              throw new Error(
                `Can not find parent route of '${br.id}',\n` + `the generated path of which is '${br.path}'.`
              )
            }
            if (!br.path.startsWith(parent.path)) {
              throw new Error(
                `The parent of '${br.id}' is '${parent.id}' in routes config.` +
                  `The generated path of '${br.id}' is '${br.path}'.` +
                  `It doesn't start with '${parent.path}' which is the generated path of '${parent.id}'.`
              )
            }
